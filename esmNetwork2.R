...
#'        
esmNetwork2 <- function(data, subjnr="subjnr", level1, level2 = NULL, vars , covs = NULL, 
  ...
  if (randomAll) { 
     ...
     (result$output <- list(graph = E, graph = G1, centralityMeasures = C, pathVariability = G2,
                            coef = coef1, model = model1))
  
  } else {
        (result$output <- list(graph = E, graph = G1, centralityMeasures = C,  
                               coef = coef1, model = model1))
    }
  
  res <- list()
  res$graph <- qgraph::getWmat(G1)
  res$results <- E
